...
const chartData: TransformedTicketThreadPoolStats[] = [
  {
    createdAt: parseDate([2023, 1, 22, 10, 30, 0, 0]).toISOString(),
    name: "testName",
    activeThreads: 0,
    idleThreads: 0,
    totalThreads: 0,
  ...
  {
    createdAt: parseDate([2023, 1, 23, 10, 30, 0, 0]).toISOString(),
    name: "testName",
    activeThreads: 0,
    idleThreads: 0,
    totalThreads: 0,
  ...
  {
    createdAt: parseDate([2023, 1, 24, 10, 30, 0, 0]).toISOString(),
    name: "testName",
    activeThreads: 0,
    idleThreads: 0,
    totalThreads: 0,
...
export default function ThreadCapacityGraph() {
  ...
  const getThreadStats = async () => {
    setIsLoading(true);
    try {
      const response = await getTicketThreadPoolStats();
      if (response.data) {
    ...
    <Card className="mt-6 w-full">
      <CardHeader className="flex flex-row w-full items-center justify-between">
        <div className="flex flex-col w-3/5">
          ...
          <CardDescription>
            Showing latest 60 instances of data collections
          </CardDescription>
        </div>
        <Button
          variant="outline"
